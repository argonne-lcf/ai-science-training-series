...
for ep in range(args.epochs):
    ...
    tt1 = time.time()
    if (hvd.rank()==0):
        print('E[%d], train Loss: %.6f, training Acc: %.3f, val loss: %.3f, val Acc: %.3f\t, Parallel threads %s,  Time: %.3f seconds ' % (ep, training_loss, training_acc, test_loss, test_acc, parallel_threads,tt1 - tt0))
        with open("results.txt", "a") as myfile:
            myfile.write(f"{hvd.size()},{ep},{training_loss},{training_acc},{test_loss},{test_acc} \n")
